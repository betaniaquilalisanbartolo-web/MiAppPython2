...
if not st.session_state["logged_in"]:
    ...
    # --- MENÚ DE USUARIO LOGUEADO (Todo debe ir indentado dentro de este else) ---
    ...
    with tab3:
        ...
        with st.form("registro_reporte"):
            ...
            spiritual_level = st.select_slider("Nivel espiritual de la reunión", options=["Bajo", "Regular", "Bueno", "Excelente"])
            attendance_level = st.slider("Porcentaje de asistencia estimada", 0, 100, 50)
    ...
    # --- Descarriados ---
    with tab4:
        st.subheader("Registro de Personas Apartadas / Descarriadas")
        conn = sqlite3.connect(DB_PATH)
        cells = pd.read_sql_query("SELECT cell_name FROM cells", conn)
        conn.close()
        cell_options = cells['cell_name'].tolist() if not cells.empty else []
        
        with st.form("registro_descarriados"):
