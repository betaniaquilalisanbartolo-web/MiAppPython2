...
# -----------------------------------------------------------------------------
st.set_page_config(
    page_title="Gestión de Iglesia - Células", page_icon="⛪", layout="wide"
)
...
def init_db():
    ...
    # Tabla de Reportes de Célula (Campos extendidos)
    cursor.execute("""
...
def obtener_lista_celulas():
    """Obtiene la lista única de células registradas tanto en miembros como en reportes."""
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("""
...
def obtener_lider_por_celula(nombre_celula):
    """Obtiene el último líder registrado o un miembro con rol de Líder para la célula seleccionada."""
    if not nombre_celula:
        return ""
    conn = get_db_connection()
    cursor = conn.cursor()
    # 1. Buscar en reportes previos
    cursor.execute(
...
def registro_miembros():
    ...
    with st.form("form_miembro", clear_on_submit=True):
        ...
        with col2:
            ...
            if opcion_celula == "+ Registrar Nueva Célula" or not celulas_existentes:
                nueva_celula_input = st.text_input(
                    "Nombre / Número de la Nueva Célula"
                )
        ...
        if guardar:
            celula_final = (
                nueva_celula_input
                if opcion_celula == "+ Registrar Nueva Célula"
                else opcion_celula
            )
            ...
            if nombre and celula_final:
                ...
                conn.close()
                st.success(
                    f"Miembro {nombre} registrado con éxito en la célula '{celula_final}'."
                )
                st.rerun()
            else:
    ...
    df_miembros = pd.read_sql_query(
        "SELECT id, nombre_completo, telefono, celula, rol, fecha_registro FROM miembros",
        conn,
    )
    conn.close()
...
def reporte_celula():
    ...
    # Selección de célula fuera del form para actualizar automáticamente el Líder sugerido
    ...
    # Sugerencia automática del líder
    lider_sugerido = (
        obtener_lider_por_celula(celula_nombre) if celula_nombre else ""
    )
    ...
    with st.form("form_reporte", clear_on_submit=True):
        ...
        with col_a1:
            asist_miembros = st.number_input(
                "Miembros", min_value=0, step=1
            )
        with col_a2:
            asist_visitas = st.number_input(
                "Visitas", min_value=0, step=1
            )
        with col_a3:
            asist_amigos = st.number_input(
                "Amigos", min_value=0, step=1
            )
        with col_a4:
            asist_ninos = st.number_input(
                "Niños", min_value=0, step=1
            )
        with col_a5:
            ofrenda = st.number_input(
                "Monto Ofrenda ($)", min_value=0.0, step=1.0
            )
        ...
        if guardar:
            if celula_nombre and lider:
                ...
                cursor.execute(
                    ...
                    (celula, lider, jefe_casa, tema_biblico, texto_biblico, fecha, 
                     asistencia_miembros, asistencia_visitas, asistencia_amigos, asistencia_ninos, ofrenda, observaciones) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                    (
                        celula_nombre,
                        lider,
                ...
                st.warning(
                    "Por favor asegúrate de especificar el nombre de la célula y del líder."
                )
...
def registro_descarrilados():
    ...
    st.info(
        "Espacio para dar seguimiento pastoral a las personas que se han distanciado de la iglesia o célula."
    )
    ...
    with st.form("form_descarrilados", clear_on_submit=True):
        ...
        if guardar:
            if nombre:
                ...
                conn.close()
                st.success(
                    "Registro añadido a la lista de consolidación/seguimiento."
                )
                st.rerun()
            else:
